...
function result = cal_proc(i, list_action1, list_coco_obj, det_root, gt_bbox, anno_im, min_overlap, num_action, format, obj_hoi_int, gt_all, all_boxes, eval_mode)
    nname = list_action1(i).nname;
    aname = [list_action1(i).vname_ing '_' list_action1(i).nname];
%    fprintf('  %03d/%03d %-30s', i, num_action, aname);
    ...
    switch eval_mode
        case 'def'
            % do nothing
        case 'ko'
            nid = cell_find_string({list_action1.nname}', nname);  %#ok
...
%    for i = 1:num_action
%        result = cal_proc(i, list_action, list_coco_obj, det_root, gt_bbox, anno_im, min_overlap, num_action, format, obj_hoi_int, gt_all, all_boxes, eval_mode);
%        ap = result(1, 1);
%        rec = result(1, 2);
%        REC(i) = rec;
%        AP(i) = ap;
%    end
    result = pararrayfun(32, @(x)cal_proc(x, list_action, list_coco_obj, det_root, gt_bbox, anno_im, min_overlap, num_action, format, obj_hoi_int, gt_all, all_boxes, eval_mode), 1:num_action, "UniformOutput", false);
