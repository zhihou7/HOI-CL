...
% dst = 'output/ho_1_s/hico_det_test2015/rcnn_caffenet_pconv_ip_iter_150000';
%dst = 'output/ho_1_s/hico_det_test2015/rcnn_caffenet_pconv_ip_R1_iter_180000';
%dect_dir = '380000_VCL_ResNet_HICO';
mkdir(dst)
% parpool(4)
pkg load parallel


function process_boxes_all (dect_dir1, dst, list1)
%    disp(obj_idx)
%    List = load('data/hico_20160224_det/anno.mat', 'list_test');
%    List = List.list_test;
%    for i = 1 : 9658
%        list(i) = str2double(List{i}(end-9 : end-4));
%    end
%    dect_dir1 = '380000_VCL_ResNet_HICO'
    Detection = load(['' sprintf('%s', dect_dir1) '/detections.mat']);

    % Detection = load(dect_file);
    ...
    for i = 1 : size(Detection,1)
        ...
        if isempty(all_boxes{HOI, find(list1 == imageId)})
            all_boxes{HOI, find(list1 == imageId)} = [H O score];
        else
            all_boxes{HOI, find(list1 == imageId)} = [all_boxes{HOI, find(list1 == imageId)}; [H O score]];
        end
...
%    disp(all_boxes)
%    all_boxes
%    save_mat(dst, all_boxes);
    save([dst '/detections.mat'], 'all_boxes');
end

%process_boxes_all(dect_dir, dst, list);

function process_boxes (obj_idx, dect_dir1, dst, list1)
%    disp(obj_idx)
%    List = load('data/hico_20160224_det/anno.mat', 'list_test');
%    List = List.list_test;
%    for i = 1 : 9658
...
%    end
%    dect_dir1 = '380000_VCL_ResNet_HICO'
    Detection = load(['' sprintf('%s', dect_dir1) '/detections_' sprintf('%02d',obj_idx) '.mat']);
    % Detection = load(dect_file);
    Detection = Detection.all_boxes;
...
%     clear all_boxes
    save_mat(obj_idx, dst, all_boxes);
end

vector_index = 1:80;
pararrayfun(32, @(x)process_boxes(x, dect_dir, dst, list), vector_index);


%for obj_inx = 1:80
%    process_boxes(obj_inx)
%end
