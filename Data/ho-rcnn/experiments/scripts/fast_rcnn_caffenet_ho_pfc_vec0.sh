...
LOG="experiments/logs/fast_rcnn_caffenet_ho_pfc_vec0.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./fast-rcnn/tools/train_net.py --gpu $gpu_id \
  ...
  --cfg ./experiments/cfgs/fast_rcnn_ho_vec0.yml \
  --iters 150000

time ./fast-rcnn/tools/test_net.py --gpu $gpu_id \
  --def ./models/fast_rcnn_caffenet_ho_pfc/test_vec.prototxt \
  --net ./output/ho_0/hico_det_train2015/fast_rcnn_caffenet_pfc_vec_iter_150000.caffemodel \
